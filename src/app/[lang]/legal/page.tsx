...
export default async function LegalPage({ params }: { params: Promise<{ lang: 'en' | 'tp' }> }) {
  const { lang } = await params;
  const safeLang = lang === 'tp' ? 'tp' : 'en';
  const dict = await getDictionary(safeLang);
  const body = Array.isArray(dict?.legal?.body) ? dict.legal.body : [];

  return (
    <main className="container" style={{ padding: '6rem 1.5rem 4rem', minHeight: '60vh', maxWidth: '800px', margin: '0 auto' }}>
      <h1 style={{ fontFamily: 'var(--font-serif)', fontSize: '2rem', marginBottom: '2rem' }}>{dict.legal.title}</h1>
